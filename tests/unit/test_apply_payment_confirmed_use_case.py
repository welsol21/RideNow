"""Unit tests for Broker application of payment confirmation."""
...
async def test_broker_applies_payment_confirmation_to_customer_visible_state() -> None:
    """Verify Broker updates stored ride state after payment confirmation."""
    ...
    use_case = ApplyPaymentConfirmedUseCase(status_store=store)
    ...
    assert store.saved["ride-1"]["status"] == "payment-confirmed"
    assert store.saved["ride-1"]["payment"] == {
        "authorisation_id": "auth-ride-1",
        "capture_id": "cap-ride-1",
        "amount": 18.5,
        "currency": "EUR",
