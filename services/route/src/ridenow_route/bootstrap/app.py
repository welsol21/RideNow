...
class RouteRuntime:
    """Real Route runtime resources."""

    publisher: RabbitMqEventPublisher
    consumer: RabbitMqEventConsumer

    async def close(self) -> None:
        """Close all runtime resources owned by Route."""

        await self.consumer.close()
        await self.publisher.close()


async def create_real_runtime(settings: SharedServiceSettings) -> RouteRuntime:
    """Create the RabbitMQ-backed Route runtime."""

    publisher = await create_event_publisher(settings.rabbitmq)
    consumer = await create_event_consumer(settings.rabbitmq)
    calculate_route = CalculateRouteUseCase(event_publisher=publisher)
    await consumer.subscribe("RouteRequested", calculate_route.execute)
    return RouteRuntime(publisher=publisher, consumer=consumer)
...
def create_app() -> FastAPI:
    ...
    settings = SharedServiceSettings(
        service_name=os.getenv("RIDENOW_SERVICE_NAME", "route")
    )
    ...
    return create_probe_app("route", lifespan=lifespan)


if __name__ == "__main__":
    run_service_app("ridenow_route.bootstrap.app", "route")
